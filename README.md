...
| 🔍 语义意图对齐 | 实时捕获用户基准意图（读取 `before_agent_start` 的 `prompt` 字段），附加本轮对话上下文，通过旁路模型检测间接提示词注入 |
| 🔗 污点追踪 | 标记外部不可信数据，阻断低完整性数据驱动高权限工具 |
| 🧩 编码载荷检测 | base64/hex/base32/URL 递归解码，检测隐藏恶意载荷 |
| 🐚 Shell 混淆检测 | 29 种 Unicode 不可见字符 + 15 种混淆模式 |
| 🤖 多模型网关 | Ollama（数据不出域）+ DeepSeek/Kimi/OpenAI 兼容协议（已验证 DeepSeek cloud API） |
...
| `fastPath.outputRedaction.enabled` | boolean | `true` | 输出内容自动脱敏（API Key、Bearer Token、手机号、身份证、银行卡号等）；双层防护：Prompt Guard 静态规则防止 LLM 在生成阶段输出敏感数据，transcript 钩子对持久化存储二次脱敏 |
...
| 输出脱敏 | 双层防护：① Prompt Guard 静态规则（`before_prompt_build`）指示 LLM 在生成阶段自主替换敏感数据，防止流式输出泄露；② `tool_result_persist` / `before_message_write` 钩子对 transcript 二次脱敏；覆盖 API Key、Bearer Token、手机号、身份证、银行卡号（银联/Visa/MasterCard/AmEx）、IMSI、内网 IP 等 | `fastPath.outputRedaction` / `fastPath.promptGuard` |
...
| 检验器 | 检测内容 | 配置节 |
|--------|----------|--------|
| 意图对齐 | Agent 计划与用户基准意图的语义一致性；LLM prompt 包含 `## 用户原始意图`（当前轮，读取 `before_agent_start.prompt`）+ `## Agent 当前计划`（`toolCallLogs` 自追踪的工具调用记录 + Fast Path 信号，v1.3.5 修复了旧方案读 `messages` 只能获取 ENOENT 错误的问题） | `slowEngine.intentAlignment` |
...
| 关键词 | 含义 |
|--------|------|
| `[ClawArmor] 拦截工具调用` | 工具调用被阻断（含 `toolName / layer / reason / flags`） |
| `[ClawArmor] 工具调用前检查` | `before_tool_call` hook 已触发（用于确认 hook 是否生效） |
| `[ClawArmor] 工具调用观察告警` | observe 模式告警，不阻断 |
| `[ClawArmor] 工具返回含嵌入式注入指令，持久化前已净化` | `tool_result_persist` 检测到 exec 注入并净化 |
| `[ClawArmor] tool_result_persist 处理完成 injectionNeutralized:true` | 注入净化链完整确认 |
| `[ClawArmor] 用户输入检测到风险模式` | 用户输入触发注入检测（含具体 flags） |
| `[ClawArmor] 数据流熔断触发` | PII + 不可信域名出站被阻断 |
| `[ClawArmor] 意图对齐：检测到意图劫持` | Slow Path `hijacked` 判定，执行阻断 |
| `[ClawArmor] 意图对齐：计划存在可疑偏离` | Slow Path `suspect` 判定，observe 告警 |
| `[ClawArmor] before_prompt_build 触发` | 每次 LLM 推理前触发，含 `toolCallLogCount`（本轮已记录工具调用数）和 `toolCallLogPreview`（前 3 条摘要）字段，用于确认 `## Agent 当前计划` 数据来源 |
| `[ClawArmor] 基准意图补充捕获（来自 before_prompt_build messages）` | `before_agent_start` 未获取到意图时，在 `before_prompt_build` 补捕 |

> **平台注意事项**：OpenClaw `api.logger` 只透传 `info` 及以上级别日志，`debug` 级日志在 gateway.log 中不可见。所有关键安全事件均使用 `info`/`warn`/`error` 级别。
